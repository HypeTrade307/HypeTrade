  ...
  useEffect(() => {
    async function fetchPortfolio() {
      try {
        ...
        const response = await axios.get(`https://hypet-145797464141.us-central1.run.app/api/portfolios/${id}`, {
  ...
  async function addStockToPortfolio(stock_id: number) {
    try {
      ...
      const response = await axios.post(
        `https://hypet-145797464141.us-central1.run.app/api/portfolios/${portfolio.portfolio_id}/stocks/${stock_id}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` }
        }
  ...
  async function uploadFile(file) {
    ...
    if (file) {
      try {
        ...
        const response = await axios.post(
            `https://hypet-145797464141.us-central1.run.app/api/portfolios/${portfolio.portfolio_id}/upload`, formData,
            {
              headers: {
